...
a scroll-driven swap happens when and how the original's does. For Gutenberg
the at-rest half is a gate: G-front's `motion` rows capture both pages at
normal motion after a scroll-through with no reveal forced, so a reveal that
never runs in WordPress (it stays at opacity 0 where the original shows it)
is a red row. When and how a reveal plays is still not gated. After gate B/C
...
|---|---|---|---|
| An element linking a page the site does not have (`journal-hook.html`) had no href in its saved HTML but `href="http://journal-hook.html"` in the theme's render: esc_url() of a plain relative href (withoutTheme red on claire `/links/`, the row's first incident) | The renderer and the save disagree on an attribute: the withoutTheme row is red. The fix is one predicate for both, never a bridge. A plain relative href is left out of the saved HTML, the editor and the render alike | none | fixed in the converter (78d994f) |
...
| Posts filed under their author (a writer's name as a category): the card's author link was read as a category | An author (a link into an author page, `rel`/`itemprop` author) is never a category: it is the post's author name (`post.author`, a stored name), which bylines bind (`postAuthorName`) where the authors differ | none | fixed in the converter (bcf4823, a5f2808) |
