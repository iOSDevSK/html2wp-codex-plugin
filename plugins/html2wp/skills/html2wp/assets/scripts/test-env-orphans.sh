...
cleanup() { for d in "$A" "$B"; do (cd "$d" && H2WP_WORKSPACE="$d" bash "$HERE/test-env.sh" down "$SLUG" >/dev/null 2>&1); done; rm -rf "$A" "$B"; }
trap cleanup EXIT
...
(cd "$A" && H2WP_WORKSPACE="$A" bash "$HERE/test-env.sh" up "$SLUG" > up1.log 2>&1) || { echo "up in A failed"; tail -20 "$A/up1.log"; exit 1; }
(cd "$B" && H2WP_WORKSPACE="$B" bash "$HERE/test-env.sh" up "$SLUG" > up1.log 2>&1) || { echo "up in B failed"; tail -20 "$B/up1.log"; exit 1; }
...
(cd "$A" && H2WP_WORKSPACE="$A" bash "$HERE/test-env.sh" up "$SLUG" > up2.log 2>&1) || { echo "second up in A failed"; tail -20 "$A/up2.log"; exit 1; }
