...
/** The listing page as the service stores it (dist-to-bundle), which is what
 * stages 4.5/4.6 wire: a page with shared chrome loses its first header and
 * trailing chrome found outside <main>; every page but the front one then
 * loses its <main> element, which WordPress renders itself. What it held
 * stays, wrapped in a <div> carrying the class and style it wore (less the
 * tokens design.templateMainClass promotes). A cardContainer that starts at
 * `main >` matches the built page and nothing that is stored (tsw, a real
 * Luna run). */
function storedView(html, page, manifest) {
  if (!page || page.kind === 'front') return html;
  return unwrapMain(page.chrome === 'self-contained' ? html : cutChrome(html, manifest), manifest);
}

function unwrapMain(html, manifest) {
  const main = findFirst(html, 'main');
  if (!main) return html;
  const attr = (name) => ((new RegExp(`\\b${name}=("([^"]*)"|'([^']*)')`, 'i')).exec(main.open) || [])
    .slice(2).find((v) => v !== undefined) || '';
  const promoted = new Set(String((manifest.design || {}).templateMainClass || '').split(/\s+/).filter(Boolean));
  const cls = attr('class').split(/\s+/).filter((c) => c && !promoted.has(c)).join(' ');
  const style = attr('style');
  let inner = main.outer.slice(main.open.length, main.outer.length - '</main>'.length);
  if (cls.trim() || style.trim()) inner = `<div${cls.trim() ? ` class="${cls}"` : ''}${style.trim() ? ` style="${style}"` : ''}>${inner}</div>`;
  return html.slice(0, main.start) + inner + html.slice(main.end);
}

function cutChrome(html, manifest) {
...
/** For a descendant selector, the one `>` path to its last step that
 * `accept` confirms on these pages; null when none or several. It is anchored
 * at the selector's first step, else at a later one (a `main` the stored page
 * no longer has), else the last step alone. */
function suggestPath(htmls, selector, accept) {
  ...
  for (const outer of parts.slice(0, -1)) {
    ...
    if (found.size) return found.size === 1 ? [...found][0] : null;
  }
  return accept(inner) ? inner : null;
}
...
export function preflight(manifest, readPage) {
  ...
  for (const [name, spec] of Object.entries(SECTIONS)) {
    ...
    // Articles and products are read off the BUILT page (build-posts,
    // build-products and make-theme all do), not the stored source.
    const details = (Array.isArray(detail) ? detail : []).map((file) => ({ file, html: readPage(file) })).filter((p) => p.html != null);
    ...
    // The article or product region: the named one, else <main>, else the
    // page. build-posts reads a blog field inside the region element itself;
    // a shop field is read inside it without the region's own tag and
    // classes (make-theme demotes them onto a wrapper).
    const regionOf = (html) => {
      const sel = named(spec.main) && parseSelector(named(spec.main)) ? named(spec.main) : 'main';
      const main = findFirst(html, sel);
      if (!main) return html;
      return name === 'blog' ? main.outer : main.outer.slice(main.open.length);
    };
    ...
    for (const field of Object.keys(FIELDS[name])) {
      ...
      const accept = where === 'listing' && field === 'cardContainer'
        ? (path) => listings.some((p) => {
          // One grid, the first match: what the cardContainer rule holds.
          const f = findFirst(p.html, path);
          return f && cardsIn(f.outer, card) > 0 && findAll(p.html, path).filter((n) => cardsIn(n.outer, card) > 0).length === 1;
        })
    ...
    for (const listing of listings) {
      ...
      if (!first) {
        // A path from <main> matches the built page and not the stored one,
        // which has no <main> (WordPress renders its own): start below it.
        const below = /^main(?![\w-])/i.test(container.trim()) ? container.replace(/^\s*main[^>]*>\s*/i, '') : '';
        const holds = (sel) => { const f = below && findFirst(listing.html, sel); return f && cardsIn(f.outer, card) > 0; };
        refuse(name, 'cardContainer', listing.file, `${name}.cardContainer "${container}" matches nothing (${measured}).`,
          below && holds(below)
            ? `The listing is stored without its <main> (WordPress renders its own): start the path below it, "${below}".`
    ...
    // Every article becomes a post only when its region and its body are
    // there (stage 4.5 skips it otherwise, and make-zip refuses the missing
    // post), and the post's title is the <h1> INSIDE the region: a region
    // that leaves the headline out imports every post untitled.
    if (name === 'blog' && !broken.has('articleMain') && !broken.has('articleBody')) {
      const mainSel = named('articleMain') || 'main';
      const bodySel = named('articleBody') || 'article';
      for (const article of details) {
        const main = findFirst(article.html, mainSel);
        if (!main) {
          refuse(name, 'articleMain', article.file, `blog.articleMain "${mainSel}" matches nothing on ${article.file}, so ` +
            `${spec.stage} skips it: it does not become a post, and make-zip refuses the theme.`,
            'Name the article region the page has (a `>` path from <main>, or the element that holds the headline and the prose).');
          continue;
        }
        const heading = findAll(article.html, 'h1').find((h) => h.outer.replace(/<[^>]*>/g, '').trim());
        if (heading && !findFirst(main.outer, 'h1')) {
          refuse(name, 'articleMain', article.file, `blog.articleMain "${mainSel}" leaves out the article's title on ` +
            `${article.file}: its <h1> is outside the region, so the post would be imported with no title.`,
            'articleMain must contain the article title: name a region that holds the <h1> and the prose.');
        }
        if (!findFirst(main.outer, bodySel)) {
          refuse(name, 'articleBody', article.file, `blog.articleBody "${bodySel}" matches nothing inside ${mainSel} on ` +
            `${article.file}, so ${spec.stage} skips it: it does not become a post.`,
            'Name the prose element inside the article region.');
        }
      }
    }
    ...
    for (const [field, where] of Object.entries(FIELDS[name])) {
      ...
      if (name === 'blog' && (field === 'articleMain' || field === 'articleBody')) continue;  // held per article above
      let pool;
      let place;
      if (where === 'card') {
