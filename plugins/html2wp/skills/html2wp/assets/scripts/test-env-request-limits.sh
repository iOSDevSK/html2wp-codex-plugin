...
# /wp-admin/ a 20 KB Cookie header: not 400. The stock command is run too, to
# show the request really is one the default refuses.
#
...
COOKIE="$(python3 -c 'print("; ".join(f"wp-preview-{i}=" + "x" * 190 for i in range(100)))')"   # ~20 KB
fail=0
cleanup() { docker rm -f h2wp-limits-probe h2wp-limits-stock >/dev/null 2>&1 || true; }
trap cleanup EXIT
cleanup

status_of() { # <container> -> the HTTP status of /wp-admin/ with the big cookie, once Apache answers
  local port code i
